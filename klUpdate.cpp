...
const int min_child_items = 16;
const int max_child_items = 32;
...
//    map<pair<int, int>, int> newMapCoreId;
    ...
    for (int i = 0; i < skylineCores.size(); i++) {
        ...
        mapCoreId[{skylineCores[i].k, skylineCores[i].l}] = i;
    }
}
...
void updatedRtreeDFS(RStarTree<int, dimensions, min_child_items, max_child_items>::Node *node, int &coreCnt,
                     ...
                     map<pair<int, int>, int> &newMapCoCore, vector<KlSkylineCore> &updatedSkylineCores,
                     vector<KlSkylineCore> &newSkylineCores) {
...
void updatedResortCores(RTree &tree,
                        map<pair<int, int>, vector<RStarTree<int, dimensions, min_child_items, max_child_items>::Node *>> &updatedMapCoNode,
                        vector<vector<int>> &updatedNodeMatrix, map<pair<int, int>, int> &updatedMapCoCore,
                        vector<vector<int>> &updatedCoreMatrix, vector<KlSkylineCore> &updatedSkylineCores,
...
//        tempKmax = max(tempKmax, updatedSkylineCores[i].k);
////        updatedCoreMatrix[updatedSkylineCores[i].k][updatedSkylineCores[i].l] = i;
//    }
...
bool compareSkylineCore2(const int &a, const int &b) {
    if (skylineCores[a].l != skylineCores[b].l) {
        return skylineCores[a].l > skylineCores[b].l;
    } else {
        return skylineCores[a].k < skylineCores[b].k;
    }
}

void addOne(RTree tree, DiGraph &g, int addNum) {
    //// load the file
    string addOnePath = "./kl-core/" + dataset + "/" + dataset + "_add";
...
//                cout << fromVertex << " " << toVertex << endl;
    ...
    for (auto tempCase: testCase) {
        ...
        RTree originTree;
        for (int i = 0; i < updatedSkylineCores.size(); i++) {
            int xx = updatedSkylineCores[i].k;
            int yy = updatedSkylineCores[i].l;
        ...
        vector<KlSkylineCore> oldSkylineCore;
        vector<KlSkylineCore> newSkylineCore;
        RTree oldTree;
        RTree newTree;
        ...
        for (auto &core: mapOldSkylineCore) {
            ...
            KlSkylineCore tempSkylineCore = KlSkylineCore();
            tempSkylineCore.k = core.first.first;
            tempSkylineCore.l = core.first.second;
            tempSkylineCore.vertices = core.second;
            oldSkylineCore.push_back(tempSkylineCore);
            mapOldCoreId[{tempSkylineCore.k, tempSkylineCore.l}] = index1;
            index1++;
        }
        for (auto &core: mapNewSkylineCore) {
            ...
            mapNewCoreId[{tempSkylineCore.k, tempSkylineCore.l}] = index2;
            index2++;
        }
        long long size = 0;
        ...
        for (auto vertex: updatedVertices) {
            ...
            for (int i = 0; i < coresAdded.size(); i++) {
                ...
                if (updatedMapCoCore.find(core) == updatedMapCoCore.end()) {
                    KlSkylineCore tempSkylineCore = KlSkylineCore();
                    tempSkylineCore.k = core.first;
                    tempSkylineCore.l = core.second;
                    ...
                    originTree.Insert(updatedSkylineCores.size() - 1,
                                      bounds(tempSkylineCore.k, tempSkylineCore.l, 0, 0));
                }
        ...
        for (int tempK = kMax; tempK >= 0; tempK--) {
            ...
            for (int tempL = klMax[tempK]; tempL >= 0; tempL--) {
                ...
                BoundingBox tempBound = bounds(tempK, tempL, kMax, 0);
                ...
                if (addedVertices.size() != 0) {
                    ...
                    vector<int> cores;
                    if (!start) {
                        bound = bounds(tempK, tempL, kMax, lMax);
                        ...
                        for (int tempA = tempK; tempA <= kMax; tempA++) {
                            ...
                            int coreId = updatedCoreMatrix[tempA][tempL];
                            if (coreId != -1) {
...
//                    }
                    ...
                    while (it != addedVertices.end()) {
                        ...
                        for (int node: g.outGraph[vertexId]) {
                            if (nodes[node]) {
                        ...
                        for (int node: g.inGraph[vertexId]) {
                            if (nodes[node]) {
                                int root2 = UF.Find(node);
                                if (root1 != root2 && addedVertices.find(node) == addedVertices.end()) {
                                    extraEdges.push_back({vertexId, node});
                    ...
                    if (updatedCoreMatrix[tempK][tempL] != -1) {
                        // updated the current core
                        bool changed = false;
                        int tempCoreID = updatedCoreMatrix[tempK][tempL];
...
//                    cout << tempCase.fromVertex << " " << tempCase.toVertex << " ";
                ...
                // updated R-tree node
                if (changedNodeCo.find({tempK, tempL}) != changedNodeCo.end()) {
                    auto RNodes = updatedMapCoNode.find({tempK, tempL})->second;
                    ...
                    changedNodeCo.erase({tempK, tempL});
                }
            }
        }
        end_time = clock();
...
void addOneMatrix(RTree tree, DiGraph &g, int addNum) {
    //// load the file
    string addOnePath = "./kl-core/" + dataset + "/" + dataset + "_add";
    addOnePath += to_string(addNum);
    ifstream addOneFile(addOnePath);
...
//                cout << fromVertex << " " << toVertex << endl;
    ...
    for (auto tempCase: testCase) {
        ...
        oldMatrix.resize(kMax + 1);
        newMatrix.resize(kMax + 1);
        int updatedKMax = 0;
        vector<int> updatedKVertexNum(kMax + 1);
        for (int i = 0; i < kMax + 1; i++) {
            oldMatrix[i].resize(klMax[i] + 1);
            newMatrix[i].resize(klMax[i] + 1);
        ...
        for (auto &core: mapNewSkylineCore) {
            ...
            if (oldMatrix[xx][yy] != -1) {
                if (oldSkylineCore[oldMatrix[xx][yy]].vertices != tempSkylineCore.vertices) {
                    updatedKMax = max(updatedKMax, xx);
                }
            } else {
                updatedKMax = max(updatedKMax, xx);
            }
//            updatedKMax = max(updatedKMax, xx);
        ...
        for (int tempK = kMax; tempK >=0; tempK--) {
            for (int tempL = klMax[tempK]; tempL >= 0; tempL--) {
                if (oldMatrix[tempK][tempL] != -1) {
                    for (auto &vertex : oldSkylineCore[oldMatrix[tempK][tempL]].vertices) {
                        tempUpdatedVertex.insert(vertex);
                    }
                }
            }
            updatedKVertexNum[tempK] = tempUpdatedVertex.size();
        }
        ...
        vector<int> preOld;
        //// Update the connectivity of the vertices in the core and Rtree Node
        for (int tempK = updatedKMax; tempK >= 0; tempK--) {
            if (tempK < kMax && klMax[tempK] == klMax[tempK + 1]) {
                bool isBlank = true;
                for (int i = 1; i <= klMax[tempK]; i++) {
                    if (updatedCoreMatrix[tempK][i] != -1 || updatedNodeMatrix[tempK][i] != -1) {
...
//            set<int> newVertices;
            ...
            for (int tempL = klMax[tempK]; tempL >= 0; tempL--) {
                ...
                for (int tempA = tempK; tempA <= kMax; tempA++) {
                    ...
                    int coreId = oldMatrix[tempA][tempL];
                    if (coreId != -1) {
                        oldCores.push_back(coreId);
                    }
                    coreId = newMatrix[tempA][tempL];
                    if (coreId != -1) {
...
void deleteOne(RTree tree, DiGraph &g, int deleteNum) {
    //// load the file
    string deleteOnePath = "./kl-core/" + dataset + "/" + dataset + "_delete";
...
//            cout << tempDelete.vertexPairs.size() << endl;
    ...
    for (auto tempCase: testCase) {
        ...
        valid++;
        //// build temp index for test
        auto updatedInGraph = g.inGraph;
        auto updatedOutGraph = g.outGraph;
        for (auto &vertexPair: tempCase.vertexPairs) {
            updatedOutGraph[vertexPair.first].erase(
                    std::remove(updatedOutGraph[vertexPair.first].begin(),
                                updatedOutGraph[vertexPair.first].end(),
                                vertexPair.second), updatedOutGraph[vertexPair.first].end());
            updatedInGraph[vertexPair.second].erase(
                    std::remove(updatedInGraph[vertexPair.second].begin(), updatedInGraph[vertexPair.second].end(),
                                vertexPair.first), updatedInGraph[vertexPair.second].end());
        }
...
//                    start_time = clock();
                    ...
                    for (int j = 0; j < tempResults.size(); j++) {
                        for (int node: updatedOutGraph[tempResults[j]]) {
                            if (nodes[node]) {
                                UF.Unite(tempResults[j], node);
                            }
                        }
                        for (int node: updatedInGraph[tempResults[j]]) {
                            if (nodes[node]) {
                                UF.Unite(tempResults[j], node);
                            }
                        }
...
//                    total_time += (double) (end_time - start_time) / CLOCKS_PER_SEC;
                    ...
                    set<int> affectedRoots;
                    for (auto item1: deletedVertices) {
                        for (int node: g.outGraph[item1]) {
                            if (nodes[node]) {
                                affectedRoots.insert(UF.Find(node));
                            }
                        }
                        for (int node: g.inGraph[item1]) {
                            if (nodes[node]) {
                                affectedRoots.insert(UF.Find(node));
                            }
                        }
                    ...
                    bool change = false;
                    if (updatedCoreMatrix[tempK][tempL] != -1) {
                        int coreId = updatedCoreMatrix[tempK][tempL];
                        // Determine whether the deleted point can change the connectivity
                        for (int j = 0; j < updatedSkylineCores[coreId].subGraphs.size(); j++) {
                            int root = UF.Find(updatedSkylineCores[coreId].subGraphs[j][0]);
                            if (affectedRoots.find(root) != affectedRoots.end()) {
                                change = true;
                                map<int, vector<int>> rootVec;
                                for (auto vertex: updatedSkylineCores[coreId].subGraphs[j]) {
...
void deleteOneMatrix(RTree tree, DiGraph &g, int deleteNum) {
    //// load the file
    string deleteOnePath = "./kl-core/" + dataset + "/" + dataset + "_delete";
    deleteOnePath += to_string(deleteNum);
    ifstream deleteOneFile(deleteOnePath);
...
//            cout << tempDelete.vertexPairs.size() << endl;
    ...
    for (auto tempCase: testCase) {
        ...
        for (int tempK = kMax; tempK >=0; tempK--) {
            for (int tempL = klMax[tempK]; tempL >= 0; tempL--) {
                if (newMatrix[tempK][tempL] != -1) {
                    for (auto &vertex : newSkylineCore[newMatrix[tempK][tempL]].vertices) {
                        tempUpdatedVertex.insert(vertex);
                    }
                }
            }
...
//                    start_time = clock();
                    ...
                    for (int j = 0; j < cores.size(); j++) {
                        ...
                        if (!nodes[firstVertex]) {
                            nodes[firstVertex] = true;
                            tempResults.push_back(firstVertex);
                            preResult.push_back(firstVertex);
                        }
                        it++;
                        while (it != vertices.end()) {
//                                UF.Unite(firstVertex, *it);
...
//                    end_time = clock();
//                    total_time += (double) (end_time - start_time) / CLOCKS_PER_SEC;
                    if (deletedVertices.size() >= newCnt) {
                        break;
                    }
                }
...
void connectivity4(DiGraph &g, RTree &tree) {
...
int main(int argc, char **argv) {
    ...
    ConstructTime += buildTime;
    std::cout << "Building R-tree Time :" << buildTime << "s" << std::endl;
    ...
    resortCores(tree);
    gettimeofday(&end_time, nullptr); // record end time
    ConstructTime += end_time.tv_sec - start_time.tv_sec + (end_time.tv_usec - start_time.tv_usec) / 1e6; // 计算时间间隔
    ...
    gettimeofday(&end_time, nullptr); // record end time
    ConstructTime += end_time.tv_sec - start_time.tv_sec + (end_time.tv_usec - start_time.tv_usec) / 1e6; // 计算时间间隔
    std::cout << "Connecting Time :" << ConstructTime << "s" << std::endl;
...
//        addOne(tree, g, nums[i]);
        addOneMatrix(tree, g, nums[i]);
        malloc_trim(0);
    }
    cout << "Delete Test:" << endl;
    for (int i = 0; i < nums.size(); i++) {
//        deleteOne(tree, g, nums[i]);
